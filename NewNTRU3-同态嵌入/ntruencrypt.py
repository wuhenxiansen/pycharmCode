...
def Owner():
	...
	encryptData=''#密文串
	while index<length:
		a=poly(plainText[index])
		a.expend(NTRU.N)
		c= NTRU.encrypto(a) #得到密文
		C.append(c)
		for ch in c.coe:
			encryptData+=chr(ch)
		index+=1
...
def DataHider(encryptdata):
	...
	secretData1=list()
	tempd=[]
	qianzhui=[0,0,0,0,0,0,0]
	while ind<=len(data1):
		tempd.append(int(data1[ind-1]))
		if ind%t==0:
			secretData1.append(qianzhui+tempd)
			tempd.clear()
		ind+=1
	if len(tempd)!=0:
		secretData1.append(qianzhui+tempd)
	...
	length=len(C)#密文长度
	length1=len(secretData1)#第一阶段总隐藏字符长度
	Binlen1 = encode(chr(len(data1)))
	...
	# 		index1+=1
	# 	index+=1
	while index<length and index<length1:
		a=poly(secretData1[index])
		a.expend(NTRU.N)
		#print('嵌入的密文：{}'.format(a.coe))
		c = NTRU.encrypto(a)  # 得到密文
		for i in range(NTRU.N):
			C[index].coe[i]+=c.coe[i]
