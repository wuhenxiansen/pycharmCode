...
plt.legend()  # 显示图例
plt.xticks(index_male, waters,fontsize=12)  # 让横坐标轴刻度显示 waters 里的饮用水， index_male + bar_width/2 为横坐标轴刻度的位置
plt.ylabel('总嵌入时间（ms）',fontsize=15)  # 纵坐标轴标题
#plt.title('不同参数下数据隐藏的总耗时')  # 图形标题
plt.savefig('')
plt.show()