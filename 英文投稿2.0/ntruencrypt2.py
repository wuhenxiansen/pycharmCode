...
def Owner():
    ...
    print('\t数据拥有者加密过程:\n')
    image = cv2.imread(r"0800.png", cv2.IMREAD_GRAYSCALE)  # 以灰度模式加载图片
    # print(image)
    ...
    secrectimage1 = cv2.imread(r"080-1.png", cv2.IMREAD_GRAYSCALE)  # 以灰度模式加载图片
...
def Receiver(C,s,rows,cols,rows1,cols1,rows2,cols2):
    ...
    #cv2.imshow("result080035", new_image)
    cv2.imwrite("result-0800~.png", new_image)

    print('隐藏数据:')
    new_image2 = addiText.reshape(rows1, cols1)
    # cv2.imshow("result080035", new_image)
    cv2.imwrite("secretImage-080-1~.png", new_image2)
def main():
    C,rows,cols,rows1,cols1=Owner()#原始图像的行列等信息可以一并加密，这里为了后续处理方便直接传递给后面的函数使用
    C,rows2,cols2,time=DataHider(C,80)
    Receiver(C,80,rows,cols,rows1,cols1,rows2,cols2)
