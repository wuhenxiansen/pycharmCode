...
def DataHider(encryptdata):
	...
	while index < length:
		...
		while index1 < NTRU.N:
			if cnt >= length2:
				break
			if cnt < length2:
				if C[index].coe[index1 ]%2!=int(data2[cnt]):
	...
	for i in range(0,16):
		...
		if C[length - 1].coe[i +16]%2!=int(Binlen2[i]):
			C[length - 1].coe[i+16] +=NTRU.p
			C[length - 1].coe[i +16] %= NTRU.q
...
def main():
	...
	# print('加密载体数据：')
	# print(encryptData)
	encryptData2,t=DataHider(encryptData)
	# print('携带隐藏数据的加密载体数据：')
	# print(encryptData2)
	Receiver(encryptData2,t)
