...
def DataHider(C, t, s):
    ...
    for filename in fileList:
        secrectimage1 = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)  # 以灰度模式加载图片
        ...
        # 处理二进制串
        secrectimage2 = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)  # 以灰度模式加载图片
...
def main():
    ...
    xishu = [[200, 200]]
    for li in xishu:
        C2, used_times = DataHider(C, li[0], li[1])
