...
def Owner():
	...
	print('\t数据拥有者加密过程:\n')
	image = cv2.imread(r"04.png", cv2.IMREAD_GRAYSCALE)  # 以灰度模式加载图片
	# print(image)
	...
	while index<length:
		...
		index+=1
	return C,rows,cols
def DataHider(C,lenB):

	t=160

	print('\t数据隐藏者嵌入数据过程:\n')

	secrectimage1 = cv2.imread(r"0800.png", cv2.IMREAD_GRAYSCALE)  # 以灰度模式加载图片
	...
	#处理二进制串
	secrectimage2 = cv2.imread(r"080.png", cv2.IMREAD_GRAYSCALE)  # 以灰度模式加载图片
	...
	while index < length:#length 密文c长度
		...
		while index1 < NTRU.N:
			...
			if cnt < length2:
				...
				# 	if C[index].coe[index1]>0:
				# 		i = 0
				# 		while C[index].coe[index1]-NTRU.p*i>-128:
	...
	return C,t,rows1,cols1,rows2,cols2
def Receiver(C,t,lenB,rows,cols,rows1,cols1,rows2,cols2):
	...
	cv2.imwrite("secretImage2-080.png", new_image1)
	#提取第一阶段数据
	ii=0
	for c in C:
		print(ii)
	...
	# cv2.imshow("result080035", new_image)
	cv2.imwrite("secretImage1-0800.png", new_image2)

	print('原始载体数据:')
	new_image = PlainText.reshape(rows,cols)
	#cv2.imshow("result080035", new_image)
	cv2.imwrite("result04521.png", new_image)
...
def main():
	...
	lenB=2
	C2,t,rows1,cols1,rows2,cols2=DataHider(C,lenB)
	# print('携带隐藏数据的加密载体数据：')
	# print(encryptData2)
	Receiver(C2,t,lenB,rows,cols,rows1,cols1,rows2,cols2)
