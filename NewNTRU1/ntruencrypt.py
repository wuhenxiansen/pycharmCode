...
def DataHider(encryptdata):
	...
	while index < length:
		...
		while index1 < 80:
			if cnt >= length2:
				break
			if cnt < length2:
				if C[index].coe[index1 + 7+80]%2!=int(data2[cnt]):
					C[index].coe[index1 + 7+80] +=1
					C[index].coe[index1 + 7+80] %= NTRU.q
				cnt += 1
			index1 += 1
		index += 1
	...
	index1 = 0
	for i in range(0,16):
		C[length-1].coe[i + 7] += int(Binlen1[i])
